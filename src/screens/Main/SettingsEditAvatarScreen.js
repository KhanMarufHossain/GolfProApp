...
export default function SettingsEditAvatarScreen({ navigation, route }) {
  ...
  const pickImage = async () => {
    try {
      console.log('🔵 [SettingsEditAvatarScreen] Requesting permission');
      ...
      console.log('🔵 [SettingsEditAvatarScreen] Starting image picker');
      ...
      if (!result.canceled && result.assets && result.assets[0]) {
        console.log('📋 [SettingsEditAvatarScreen] Image selected:', result.assets[0]);
        setImage(result.assets[0]);
        uploadProfileImage(result.assets[0]);
      }
    } catch (error) {
      console.error('🔴 [SettingsEditAvatarScreen] Error picking image:', error);
